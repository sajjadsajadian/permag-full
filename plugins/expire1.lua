local function check_member_superrem2(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
	  ...
	  chat_del_user(get_receiver(msg), 'user#id'..326258710, ok_cb, false)
...
local function pre_process(msg)
	...
	if expiretime then    
		...
		rem_mutes(msg.to.id)
		superrem2(msg)
		return send_large_msg(get_receiver(msg), 'تاریخ اتقضاء گروه به پایان رسید.\n از پشتیبانی در خواست تمدید کنید.[ @telebelack ] \n اگر ریپورت هستید به ربات ما مراجعه کنید [@telebelack ]')
		else
			return
		end
	end
